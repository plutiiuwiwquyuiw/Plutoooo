...
const name = info[event.senderID].name
    ...
    if(msgData[event.messageID].attachments.length === 0) {
        api.sendMessage(`${name} UNSEND PA: ${msgData[event.messageID].body}`, event.threadID)  
      ...
      for (const item of msgData[event.messageID].attachments) {
       let { data } = await axios.get(item.url, {responseType: "arraybuffer"})
          ...
          del.push(`./script/cache/${item.filename}.jpg`)
                    ...
                    api.sendMessage({body:`${name} NABOANG KANA BA??: ${msgData[event.messageID].body}`, attachment: photo}, event.threadID, () => {
...
api.sendMessage({body:`${name} OHH SHEESH: ${msgData[event.messageID].body}`, attachment: fs.createReadStream('./script/cache/audio.mp3')}, event.threadID, () => {
...
api.sendMessage({body:`${name} GAGI NAGALAW: ${msgData[event.messageID].body}`, attachment: fs.createReadStream('./script/cache/animated_image.gif')}, event.threadID, () => {
